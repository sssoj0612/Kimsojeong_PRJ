...
public class NoticeController {
    ...
    public String noticeInsert(HttpServletRequest request, ModelMap model, HttpSession session) {
        ...
        try { // 로그인 된 사용자 아이디를 가져오기
            ...
            String user_id = kopo.kimsojeong_prj.util.CmmUtil.nvl((String) session.getAttribute("SS_USER_ID"));
            String title = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("title")); // 제목
            String notice_yn = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("notice_yn")); // 공지글 여부
            String contents = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("contents")); // 내용
    ...
    public String noticeInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        String nSeq = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("nSeq")); // 공지글번호(PK)
    ...
    public String noticeEditInfo(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        String nSeq = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("nSeq"));
    ...
    public String noticeUpdate(HttpSession session, ModelMap model, HttpServletRequest request) {
        ...
        try {
            String user_id = kopo.kimsojeong_prj.util.CmmUtil.nvl((String) session.getAttribute("SS_USER_ID")); // 아이디
            String nSeq = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("nSeq")); // 글번호
            String title = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("title")); // 제목
            String notice_yn = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("notice_yn")); // 공지글 여부
            String contents = kopo.kimsojeong_prj.util.CmmUtil.nvl(request.getParameter("contents")); // 내용
